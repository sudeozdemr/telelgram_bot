...
async def fetch_url_info_command(url: str) -> tuple:
    try:
        ...
        return None, None, None, f"Error retrieving content from URL: {e}"
...
async def scheduled_task(bot: Bot):
    ...
    for user_id, data in list(user_data.items()):
        ...
        try:
            ...
            if current_time - query_time > timedelta(minutes=20):
                del user_data[user_id]
...
def main():
    ...
    # Scheduler
    scheduler.start()
    scheduler.add_job(scheduled_task, IntervalTrigger(seconds=10), args=[bot])
